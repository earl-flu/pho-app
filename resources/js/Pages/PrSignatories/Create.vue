...
const submit = () => {
  form.post(route("pr-signatories.store"), {
    onSuccess: () => {
      const toast = useToast();
      toast.success("Pr Signatory Saved Successfully", {
        timeout: 3000,
      });
    },
  });
...
watch(error, (newError) => {
  if (newError.saving && Object.keys(newError.saving).length > 0) {
    useToast().error(newError.saving, {
      timeout: 3000,
    });
  }
});
